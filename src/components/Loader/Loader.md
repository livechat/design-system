...
Loaders help reduce the use of awkward blank screens. They’re CSS animated SVGs telling people that the app is loading data or processing a task, such as saving or submitting user-inputted data. You should use only one spinner on a page at a time. Don’t show spinners if a task takes less than 0.3 s to process.
