...
return (
  <div>
    ...
    <UserGuide
      ...
      elementClassName={/* custom class name for highlighted element */}
    >
...
Component allows to easily style the highlighted element by passing a custom className. To do this, you need to provide the `UserGuide` component with a `elementClassName` prop:
...
You can use your state management to apply different class names for highlighted elements, the component will update accordingly.
...
When adding images or videos to UserGuideStep, keep in mind the following constraints:

- Maximum height: 240px
- Maximum width: 100% of the container (340px) (380px full width - 2*20px for padding)
- Media will maintain its aspect ratio while scaling
- Supported via `image` or `video` props:
...
For best results:
- Optimize images to be close to the display size (380px width)
- Use 16:9 or 4:3 aspect ratios for consistent appearance
- Images/videos that are too tall will be scaled down to 240px height
...
`UserGuideBubbleStep` is a variant of the guide step that displays content in a bubble format with celebration animations. Used inside the `UserGuide`, it provides a different visual style for highlighting features.
