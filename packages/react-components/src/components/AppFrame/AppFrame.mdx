...
`NavigationTopBar` provides no properties, but allows children, notably `NavigationTopBarAlert` (`NavigationTopBar.Alert`). 
...
- `show` - defines whether the alert is visible.
