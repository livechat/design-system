...
- `isSideNavigationBarVisible` - defines whether side navigation is open/closed
- `toggleSideNavigationBarVisibility` - function changing the value of `isSideNavigationBarVisible`
...
const SideNavigationSwitcher: FC = () => {
  const { isSideNavigationBarVisible, toggleSideNavigationBarVisibility } = useAppFrame();
