...
By default, items in the left column are centered, while items in the right column are aligned to the bottom right. The component contains a breakpoint depending on the width of the main container, which, if it is <b>less than 559px</b>, will change the view to "vertical" and the contents of the right column will be moved above the left column. In "vertical" view, the contents of the right column will be left-aligned by default, the left column will remain unchanged. If you want more control of these elements (left and right columns) in both views, below is an instruction on how to control the position of individual elements via CSS styles.
