...
<Column sm={2} lg={8}>
  ...
  {/* Takes 8 columns (50%) on large screens and up */}
</Column>
```

3. **Default Behavior**
- By default, columns have `flex: 1 1 auto` and `max-width: 100%`
- When a breakpoint is specified, it sets a fixed width percentage using `flex: 0 0` and corresponding `max-width`
...
1. **Breakpoint-based Sizing**
   ...
   - Cannot be combined with width/minWidth/maxWidth

2. **Direct Width Control**
   - Best for fixed-width columns
   - Can be combined with minWidth/maxWidth for constraints
