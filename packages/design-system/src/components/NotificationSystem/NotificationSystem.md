    ...
    <div>
      <div style={{ marginBottom: "15px" }}>
        ...
        <select
          ...
          onChange={event => setState({ vertical: event.target.value })}
        >
      ...
      <div style={{ marginBottom: "15px" }}>
        ...
        <select
          ...
          onChange={event => setState({ horizontal: event.target.value })}
        >
      ...
      <ToastedButton>
        {state.currentToastId ? "Hide" : "Show"} toast
      </ToastedButton>
    </div>
  </NotificationProvider>
</div>;
...
const ButtonWithToast = ({ notificationSystem, children }) => {
  const createRandomToast = () => {
    ...
    const randomHorizontalPosition =
      horizontalPositions[
        Math.floor(Math.random() * horizontalPositions.length)
      ];
    const randomVerticalPosition =
      verticalPositions[Math.floor(Math.random() * verticalPositions.length)];
