...
const options = [
  { id: "today", label: "Today", value: { from: new Date(), to: new Date() } },
  {
    ...
    value: {
      ...
      to: DateFns.subDays(new Date(), 1)
    }
  },
  {
    id: "last_7_days",
    label: "Last 7 days",
    value: { from: DateFns.subDays(new Date(), 7), to: new Date() }
  },
...
const getSelectedItemBody = (props, inputs) => {
  if (props.isManual) {
    return <DatePickerRangeSelectInputs {...inputs} />;
  }
  return <SelectItem id={props.value}>{props.label}</SelectItem>;
};

const getItemBody = props => (
  <SelectItem id={props.value}>{props.label}</SelectItem>
);
      ...
      <React.Fragment>
        <SelectField
          ...
          getSelectedItemBody={selectedItemProps =>
            getSelectedItemBody(selectedItemProps, inputs)
          }
